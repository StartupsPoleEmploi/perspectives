...
class MRSValideesCSVLoaderSpec extends AsyncWordSpec
  ...
  val mrsValideeCSVLoader: MRSValideesCSVLoader = new MRSValideesCSVLoader(actorSystem)
  ...
  "integrerMRSValidees" should {
    "ne rien faire si aucune ligne n'est présente dans la source" in {
      ...
      // When
      val future = mrsValideeCSVLoader.load(source)
